...
```bash
npx componentry@latest add magnetic-dock
```
...
|------|------|---------|-------------|
| `items` | `DockItem[]` | - | Array of dock items with id, label, icon, onClick, isActive, and badge. (Required) |
| `iconSize` | `number` | `56` | Base size of icons in pixels. |
| `maxScale` | `number` | `1.5` | Maximum scale factor when hovering directly over an icon. |
| `magneticDistance` | `number` | `150` | Pixel distance for magnetic effect falloff. |
| `showLabels` | `boolean` | `true` | Show tooltip labels on hover. |
| `position` | `"bottom" | "top" | "left" | "right"` | `"bottom"` | Dock orientation. |
| `variant` | `"glass" | "solid" | "transparent"` | `"glass"` | Background style variant. |
...
_Component from [Componentry](https://componentry.fun/docs/components/magnetic-dock)_
